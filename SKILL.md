...
| Setting `_element_width` to `"custom"` when specifying custom widget widths | Always set `_element_width` to **`"initial"`** (which is the actual value for the "Custom" select option in Elementor's database schema) and define the size under `_element_custom_width`. Setting it to `"custom"` compiles the word `custom` literally and breaks the layout. |
...
| Leaving Grid Rows empty for single-row CSS Grid containers | Always explicitly define the row count (e.g. `{"size": 1, "unit": "fr"}`) under `grid_rows_grid`. Leaving it empty triggers Elementor's core default fallback of 2 rows (`repeat(2, 1fr)`), which reserves empty vertical height space. |
